...
That `L9H8`'s patched effect is *tiny* is the interesting part: its output barely changes between the two frames, which is what frame-*independent* retrieval would look like — the head writes `·Paris` in both, and the gate lives in the final-token representation the competing MLPs read. So DLA and the patch agree only on what each can see — a direct writer and a frame lever — and whether they are one mechanism or two needs an intervention the appositive donor can't give: a donor matched on the final token, or path patching that isolates `L9H8` → logit.
