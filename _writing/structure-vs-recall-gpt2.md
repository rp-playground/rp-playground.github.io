...
So **computation, writer, and gate are three different places**: `L9H8` writes `·Paris`; the MLP stack suppresses it — directly, and by routing `L9H8`'s own output back against it; and the final-token frame is the lever the activation patch was really moving.
