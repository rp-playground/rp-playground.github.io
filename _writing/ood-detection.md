...
the model's own confidence separates real bears from non-bears), then try to
...
- **Two OOD regimes:**
  ...
  - **near-OOD**: [Oxford-IIIT Pet](https://www.robots.ox.ac.uk/~vgg/data/pets/):
    ...
    and corrupt the metric). This near-OOD scenario exposes where the detector actually fails.
...
**2. Near-OOD is where it falls apart, and MSP, the plain baseline, beats energy.**
...
a tiny 3-class head fine-tuned on a few hundred bears. And the takeaway isn't
...
- On **near-OOD**, where the detector actually has to work, it's weak, and on this small
