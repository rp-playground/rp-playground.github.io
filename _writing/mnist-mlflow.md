...
It covers tracking, hyperparameter search, the model registry, and the artifacts each run
...
<figure class="narrow">
  ...
  <figcaption>Optuna's fANOVA importance. With only 4 completed trials it's noisy: here <code>dropout</code> and <code>conv_channels</code> tie (~0.36) and <code>lr</code> trails (0.28), and the ranking wobbles if you rerun it.</figcaption>
</figure>
