---
layout: article
title: "DanteGPT Technical Report 0023–0030: Reranker Fine-Tuning and Commentary Indexing"
subtitle: "Machine-generated from the DanteGPT ML journal. How the bottleneck moved from recall to ranking, and why five of the eight experiments were negative results."
description: A machine-generated technical report over decision records 0023–0030 of the DanteGPT ML journal. Claude Code (Opus 4.8) read the records via the /report skill and wrote this from them, transcribing every number from the record that produced it. Reranker fine-tuning gave a significant but saturating transfer gain; late fusion, RRF-mined negatives, ranking loss, entity-aware rerank passages, and a query rewriter all failed their pre-registered criteria; entity docs and Chiavacci commentary docs were adopted. The bottleneck is now the ranking stage, not the candidate pool.
summary: "This report is machine-generated. Claude Code (Opus 4.8) read decision records 0023–0030 of the DanteGPT ML journal through the /report skill and wrote the piece from them, transcribing every number from the record that produced it rather than re-deriving anything. The style guide it wrote under is mine; the analysis and the verdicts are the records'. What the eight records hold: three adoptions and five negative results. Fine-tuning the cross-encoder reranker beats zero-shot by +0.042 Recall@1 on a human-validated transfer set (p=0.006), but top-1 saturates at the smallest data scale, and every training-side lever tried afterwards failed its pre-registered criterion. Commentary notes indexed as extra tercet representations lifted pool recall@50 from 0.813 to 0.864, yet the reranker promotes only 1 of the 29 newly retrieved gold tercets to rank 1, so the bottleneck has moved from recall to ranking."
date: 2026-07-21
...
> is transcribed from the record that produced it. The pipeline, and how it is
> invoked, is described in [a separate
> piece](/writing/ml-journal-reporting-pipeline/).
